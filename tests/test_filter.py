...
class TestFilter:
    ...
    def test_filter_row(self):
        filtered_data = get_data(
            self.test_file, start_row=3, library="pyexcel-odsr"
        )
    ...
    def test_filter_row_2(self):
        filtered_data = get_data(
            self.test_file, start_row=3, row_limit=1, library="pyexcel-odsr"
        )
        expected = [[4, 24, 34]]
        eq_(filtered_data[self.sheet_name], expected)
    ...
    def test_filter_column(self):
        filtered_data = get_data(
            self.test_file, start_column=1, library="pyexcel-odsr"
        )
    ...
    def test_filter_both_ways(self):
        filtered_data = get_data(
            self.test_file, start_column=1, start_row=3, library="pyexcel-odsr"
        )
